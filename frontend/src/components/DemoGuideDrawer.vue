...
const commonSteps = computed(() => [
  ...
  ...(props.showResetTip ? ["如果中途想回到初始状态，可在右上角点击“重置本地 Mock 数据”。"] : []),
]);
...
const roleGuide = computed(() => {
  if (props.roleCode === "ADMIN") {
    return {
      title: "管理员演示路径",
      purpose: "说明系统当前后台能力如何支撑配置治理、账号维护与审计查看。",
      steps: [
        "从仪表盘查看首页总览，确认管理员不会误入业务处理链。",
        "进入“指标规则”，查看指标、权重分布、评分区间与启停动作。",
        "进入“用户管理”，确认当前登录用户保护与管理员维护能力已经接通。",
        "进入“操作日志”，筛选查看指标规则、用户管理等后台写操作记录。",
      ],
    };
  }
